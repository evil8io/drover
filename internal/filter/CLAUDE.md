...
- Verified clients: kubectl, k9s, OpenLens, and Aptakube (chunked). Headlamp (websocket) is not verified until a live check of the per-message form. No integration test drives the websocket path, so a change to `websocket.go` needs a live check with Headlamp against a cluster whose API server sends plain JSON.
